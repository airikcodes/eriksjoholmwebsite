...
//
// data-theme = "light" always (the bright design); data-tone = "day" | "night" -> paper/ink darkened at night (flip at daylight 0.5). The old black dark theme lives on branch dark-theme-archive.
...
export const THEME_SCRIPT = `(function(){
  ...
  function apply(){
    ...
    // The design is always the bright one; night only changes the tone (paper + ink), never the layout.
    var theme='light', tone='day', wash=0.62;
    if(mode==='dark'){tone='night'}
    else if(mode==='auto'){var dl=daylight(new Date());tone=dl>=0.5?'day':'night'}
    var prev=d.getAttribute('data-tone');
    if(prev&&prev!==tone){d.classList.add('theme-fade');setTimeout(function(){d.classList.remove('theme-fade')},1600)}
    d.setAttribute('data-theme',theme);
    d.setAttribute('data-tone',tone);
    ...
    m.setAttribute('content',tone==='night'?'#2A2622':'#F7F3EE');
